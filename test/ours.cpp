...
int main(int argc, char** argv) {
  ...
  // INPUT+OUdpUT+MULT.
  PRINT("fd_prep SEND");
  // std::thread t_PrepMuldpartiesSendP1( &dp::Circuit::PrepMuldpartiesSendP1, &circuit ); 
  // std::thread t_PrepMuldp1ReceivesAndSends( &dp::Circuit::PrepMuldp1ReceivesAndSends, &circuit ); 
  // t_PrepMuldpartiesSendP1.join();
  // t_PrepMuldp1ReceivesAndSends.join();
  ...
  std::vector<dp::FF> result;
  if (id == 0){
    std::vector<dp::FF> inputs{dp::FF(0432432), dp::FF(54982)};
