...
TEST_CASE("InputGate") {
  ...
  SECTION("InputGate") {
    ...
    dp::FF lambda_x(50942);
    dp::FF lambda_y(-4523);

    for (std::size_t i = 0; i < n_parties; i++) {
      x_gates.emplace_back(std::make_shared<dp::InputGate>(0)); // P1 owner
      x_gates[i]->SetNetwork(std::make_shared<scl::Network>(networks[i]), i);
      x_gates[i]->_DummyPrep(lambda_x);

      y_gates.emplace_back(std::make_shared<dp::InputGate>(1)); // P2 owner
    ...
    dp::FF mu_x = x_gates[0]->GetMu();
    dp::FF mu_y = y_gates[0]->GetMu();
